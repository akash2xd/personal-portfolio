...
const SkillItem = React.memo(({ skill }: { skill: string }) => {
  const Icon = getSkillIcon(skill);
  const color = getSkillColor(skill);

  return (
    <div className="mx-4 my-4 group">
      <div className="relative px-8 py-6 rounded-2xl border border-white/5 bg-white/[0.02] backdrop-blur-md transition-all duration-500 hover:border-indigo-500/30 hover:bg-white/[0.05] flex items-center gap-4 shadow-xl">
        <div
        ...
        <div className="flex flex-col">
          <span className="text-sm font-bold text-white tracking-wide uppercase group-hover:text-indigo-500 transition-colors">
            {skill}
          </span>
...
function Skills() {
  ...
  return (
    <div id="skills" className="relative z-50 py-24 lg:py-48 overflow-hidden ">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-indigo-600/10 blur-[80px] rounded-full pointer-events-none" />
      <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-cyan-900/10 blur-[80px] rounded-full pointer-events-none" />
