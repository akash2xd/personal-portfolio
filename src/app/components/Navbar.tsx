  ...
  const renderLink = (
    ...
    if (isHomePage) {
      return (
        ...
        >
          {item.label}
          {!isMobile && (
            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-indigo-600 to-cyan-900 transition-all duration-300 group-hover:w-full [.text-indigo-400_&]:w-full" />
          )}
        </ScrollLink>
      );
    }
    ...
    return (
      ...
      >
        {item.label}
        {!isMobile && (
          <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-indigo-600 to-cyan-900 transition-all duration-300 group-hover:w-full" />
        )}
      </Link>
    );
  };
